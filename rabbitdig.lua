...
Rabbit is a computercraft mining program that digs vertically and checks every face of the tunnel it mines. This is in order to get every possible material from every possible
layer. This totally ruins the environment so I reccomend doing it somewhere far away from your usual location.

It will automatically deposit your items in an enderchest when it's full and move to the next hole location.

It will automatically do only one hole in a line so it's really meant to be run in parallel with multiple other mining programs.

Automatic refuel? No clue... Maybe just fill it up for a while then it will run and stop after a bit? Not sure. Honestly I may not even need to make this since
the recursive plus program works pretty well.
This does work better than plus in terms of getting all the items possible in a smaller area so less chunk loading...

Thoughts about just using this:
http://www.computercraft.info/forums2/index.php?/topic/7663-rebootable-rabbit-hole-mining-program/
https://pastebin.com/JbFMHaNg
...
a, b, c, d = turtles all started facing forwards (up in the illustration)
1, 2, 3, 4 = collumns of blocks all mined by a, b, c, d respectively.
